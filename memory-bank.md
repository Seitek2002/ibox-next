...
- src/app/[venue]/layout.tsx
  ...
  - Sets Metadata: title (companyName — ishop), description, icons (logo), OpenGraph/Twitter.
...
- Current usage (to be refactored/centralized if backend WS changes):
  - src/components/Hero/index.tsx: new WebSocket(`wss://ishop.kg/ws/orders/?phone_number=${user.phoneNumber}`)
  - src/client-pages/Order/index.tsx: new WebSocket(`wss://ishop.kg/ws/orders/?phone_number=${user.phoneNumber}&site=imenu`)
- Recommendation:
...
- Marketing page includes links/emails with ishop.kg:
  - Demo link: https://ishop.kg/Exponenta
  - Email: info@ishop.kg
  - Footer copy: © {year} ishop.kg
...
- Changed default REST base from https://ishop.kg/api/ to https://stark.adamtech.dev/api/
  - src/api/base.ts updated.
  - src/app/[venue]/layout.tsx SSR fetch updated to use env + fallback.
- Hard-coded WS endpoints remain at ishop.kg and should be centralized if backend WS origin changes.
